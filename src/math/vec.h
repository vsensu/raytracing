...
public:
    ...
    vec(typename std::enable_if<sizeof...(Tail) + 1 == N, ElementType>::type head, Tail... tail)
    ...
    auto operator-() const
    {
        return minus(std::make_index_sequence<N>{});
    }
...
private:
    ...
    template <std::size_t... I>
    void transform_inner(std::function<void(const ElementType &)> p, std::index_sequence<I...>) const
    {
        ((p(elems_[I])), ...);
    }
    ...
    template <std::size_t... I>
    void transform_inner(std::function<void(ElementType &)> p, std::index_sequence<I...>)
    {
        ((p(elems_[I])), ...);
    }
